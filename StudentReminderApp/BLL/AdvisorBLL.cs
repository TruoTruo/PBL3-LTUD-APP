// BLL/AdvisorBLL.cs (Phiên bản không dùng GetPrerequisites)
    ...
    public class AdvisorBLL
    {
        ...
        public List<LopHocPhan> GetSuggestedCourses(long idSv, int hocKy, string namHoc)
        {
            ...
            foreach (var lhp in available)
            {
                ...
                // Cách 1: Nếu có method GetPrerequisites trong DAL
                // var prereqs = _advisorDal.GetPrerequisites(lhp.IdMonHoc);
                // if (prereqs.All(p => passed.Contains(p)))
                //     result.Add(lhp);

                // Cách 2: Tạm thời bỏ qua kiểm tra tiên quyết
                // Chỉ thêm vào danh sách gợi ý nếu chưa học và chưa đăng ký
                result.Add(lhp);
            }
            return result;
        }
